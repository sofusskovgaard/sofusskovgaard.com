  ...
  return (
    ...
    >
      <h5 className="font-semibold text-lg">
        <Link
          href={`/blog/${doc.uid}`}
          className="underline group-hover:text-decoration-style-dotted focus:text-decoration-style-dotted decoration-2"
        >
          {doc.data.title[0].text}
        </Link>
      </h5>
      <p className="text-gray-600 text-sm">{doc.data.subtitle[0].text}</p>
      <small className="block uppercase text-gray-500">
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col p-4 absolute top-0 bottom-0 left-0 right-0 transition-all duration-200 group-hover:backdrop-blur-sm backdrop-blur-0">
        ...
        <small className="block uppercase text-gray-50 mt-auto ml-auto">
          {formatDate(doc.first_publication_date)}
        </small>
      </div>
    </article>
