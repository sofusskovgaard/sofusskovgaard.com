...
const BlogPost = observer(
  ({ post, latestPosts, nextPost, prevPost }: Props) => {
    ...
    return (
      <Container>
        <Head>
          <title>
            {post.data.title[0].text} — {stores.uiStore.app_name}
          </title>
          ...
          {post.data.thumbnail.url != null && (
            <meta name="twitter:image:src" content={post.data.thumbnail.url} />
          )}
          ...
          <meta
            property="og:url"
            content={`https://sofusskovgaard.com/blog/${post.uid}`}
          />
          {post.data.thumbnail.url != null && (
          ...
          {post.first_publication_date && (
            <meta
              ...
              content={new Date(post.first_publication_date).toISOString()}
            />
          )}
        </Head>
        ...
        <div className="flex flex-col gap-4">
          {(nextPost != null || prevPost != null) && (
            <div className="flex">
              ...
              {prevPost != null && (
                <Link
                  href={`/blog/${prevPost.uid}`}
                  className="text-gray-600 hover:text-black hover:underline text-sm ml-auto"
                >
                  Previous post &#8594;
                </Link>
              )}
          ...
          <div>
            <p className="block font-semibold text-gray-500">
              {formatLongDate(post.first_publication_date)}
            </p>
        ...
        <div className="flex flex-col md:flex-row gap-10 items-start">
          <section className="prose flex-2">
            ...
            <aside>
              <div className="p-4">
                ...
                <small className="hidden md:block">
                  Feel free to keep browsing, here are some of my latest posts.
...
export async function getStaticPaths(): Promise<GetStaticPathsResult> {
  const posts = (await PrismicService.getBlogPosts()) as ApiSearchResponse;
  return {
